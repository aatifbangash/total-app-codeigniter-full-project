...
class Administrator extends CI_Controller
{
	...
	public function add_quiz($page = 'add-quiz')
	{
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
			show_404();
		}
		...
		if ($this->form_validation->run() === FALSE) {
			...
			//Upload Image
			$this->Administrator_Model->add_quiz();

			//Set Message
			$this->session->set_flashdata('success', 'Quiz has been created Successfull.');
			redirect('administrator/quiz');
		}
	}

	public function submit_quiz($page = 'add-quiz')
	{
		if (!empty($this->input->post())) {
			foreach ($this->input->post() as $question => $answer) {
				$questionId = str_replace('q-', '', $question);
				$answerId = $answer;
				echo $answerId;
			}
		}
	...
	public function add_answer($qid = null)
	{
		// working hre
		$page = 'add-answer';
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
		...
		$data['title'] = 'Create Answer';
		$data['qid'] = $qid;

		$this->form_validation->set_rules('answer', 'Answer', 'required');
		if (empty($qid)) {
			$data['qid'] = $table = $this->input->post('qid');
			$qid = $data['qid'];
		}
		if ($this->form_validation->run() === FALSE) {
			$this->load->view('administrator/header-script');
			...
			//Upload Image
			$this->Administrator_Model->add_answer();

			//Set Message
			$this->session->set_flashdata('success', 'Answer has been created Successfull.');
			redirect('administrator/answers/' . $qid);
		}
	}
	...
	public function quiz($offset = 0)
	{
		// Pagination Config
		$config['base_url'] = base_url() . 'administrator/quiz/';
		$config['total_rows'] = $this->db->count_all('quiz');
		$config['per_page'] = 3;
		...
		$data['title'] = 'Latest Quiz';

		$data['quizes'] = $this->Administrator_Model->get_quizs(FALSE, $config['per_page'], $offset);
		$this->load->view('administrator/header-script');
		$this->load->view('administrator/header');
		$this->load->view('administrator/header-bottom');
	...
	public function answers($questionId)
	{
		...
		$data['question'] = $this->Administrator_Model->get_quizs($questionId);
		$data['answers'] = $this->Administrator_Model->get_answers($questionId);
		$this->load->view('administrator/header-script');
		$this->load->view('administrator/header');
		$this->load->view('administrator/header-bottom');
		$this->load->view('administrator/answers', $data);
		$this->load->view('administrator/footer');
	}
	...
	{
		$data['quiz'] = $this->Administrator_Model->get_quizs($id);
		...
		$this->load->view('administrator/header');
		$this->load->view('administrator/header-bottom');
		$this->load->view('administrator/update-quiz', $data);
		$this->load->view('administrator/footer');
	}
	...
	{ //working here
		...
		$data['answer'] = $this->Administrator_Model->get_answer($id);

		if (empty($data['answer'])) {
			show_404();
		}
		$data['title'] = 'Update Answer';;
		$this->load->view('administrator/header-script');
		$this->load->view('administrator/header');
		$this->load->view('administrator/header-bottom');
		$this->load->view('administrator/update-answer', $data);
		$this->load->view('administrator/footer');
	}

	public function update_quiz_data($page = 'update-quiz')
	{
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
			show_404();
		}
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Administrator_Model->update_quiz_data();
			//Set Message
	...
	public function update_answer_data($page = 'update-answer')
	{
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
			show_404();
		}
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->Administrator_Model->update_answer_data();
			//Set Message
			$this->session->set_flashdata('success', 'Answer has been Updated Successfull.');
			redirect('administrator/answers/' . $this->input->post('qid'));
		}
	}
